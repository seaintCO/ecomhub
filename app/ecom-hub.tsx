...
function ProductLab({ add }: { add: (product: typeof researchProducts[number]) => void }) {
  ...
  return <><Title eyebrow="PRODUCT INTELLIGENCE" title="Product Lab" text="Shortlist ideas, compare economics and open real research sources before committing capital." action={<a className="dark-btn" target="_blank" href={`https://trends.google.com/trends/explore?q=${encoded}`}>Open live trends ↗</a>} /><div className="product-search">⌕<input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search curated product candidates..." /></div><div className="sources"><a target="_blank" href="https://www.amazon.com/gp/movers-and-shakers">amz <b>Amazon Movers</b> ↗</a><a target="_blank" href="https://ads.tiktok.com/business/creativecenter/inspiration/popular/pc/en">tt <b>TikTok Creative</b> ↗</a><a target="_blank" href={`https://www.facebook.com/ads/library/?q=${encoded}`}>meta <b>Meta Ad Library</b> ↗</a><a target="_blank" href={`https://www.aliexpress.us/w/wholesale-${encoded}.html`}>ali <b>AliExpress Search</b> ↗</a></div><p className="data-note"><i>i</i><span><b>Curated training data</b> · Scores are examples for learning the workflow. Verify live demand, suppliers and saturation using the sources above.</span></p><div className="products"><header><span>PRODUCT</span><span>SCORE</span><span>COST / PRICE</span><span>VELOCITY</span><span>SATURATION</span><span /></header>{results.map((p) => <article key={p.name}><p><i>{p.category[0]}</i><span><b>{p.name}</b><small>{p.category}</small></span></p><strong>{p.score}</strong><span>{cash(p.cost)} / {cash(p.price)}</span><b className="green">{p.velocity}</b><em className={p.saturation.toLowerCase()}>{p.saturation}</em><button onClick={() => add(p)}>Add to tracker +</button></article>)}</div></>;
}

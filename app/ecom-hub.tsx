...
import { flashcards, modules, researchProducts, type Lesson } from "./course-data";
...
const quiz = [
  { q: "Which metric shows what remains after product, shipping, fees and advertising?", a: ["Revenue", "Contribution margin", "Sessions", "CTR"], correct: 1 },
  { q: "What should happen before scaling a product?", a: ["Buy 1,000 units", "Copy a competitor", "Validate demand and economics", "Lower the price"], correct: 2 },
  { q: "What is the healthiest supplier validation step?", a: ["Trust the listing", "Order a sample", "Choose the cheapest", "Skip return terms"], correct: 1 },
];
...
function LessonPlayer({ id, completed, toggle, back, change }: { id: string; completed: string[]; toggle: (id: string) => void; back: () => void; change: (id: string) => void }) {
  ...
  const video = current.module.id === 2 ? "DRgCOONskcA" : current.module.id > 3 ? "0hJG0OdvRwY" : "YImqGsOPqU8";
  const navLesson = (lesson?: Lesson) => { if (!lesson) return; change(lesson.id); setTab(lesson.kind === "Quiz" ? "Quiz" : "Notes"); };
  const score = Object.entries(answers).filter(([q, a]) => quiz[Number(q)].correct === a).length;
  return <><button className="back" onClick={back}>← Back to curriculum</button><div className="lesson-layout"><main><div className="video"><iframe src={`https://www.youtube-nocookie.com/embed/${video}?rel=0&modestbranding=1`} title={current.title} allowFullScreen /></div><div className="lesson-title"><small>MODULE {String(current.module.id).padStart(2, "0")} · {current.kind.toUpperCase()}</small><h1>{current.title}</h1><p>{current.module.summary} Apply the lesson with the action plan below.</p></div><div className="tabs">{(["Notes", "Flashcards", "Quiz"] as const).map((t) => <button className={tab === t ? "active" : ""} key={t} onClick={() => setTab(t)}>{t}</button>)}</div>
      {tab === "Notes" && <section className="notes"><h3>Lesson action plan</h3>{[["Choose one decision.", "Write the single choice this lesson helps you make."], ["Gather evidence.", "Use customer, competitor and supplier evidence—not assumptions."], ["Define the next test.", "Choose a low-risk action you can complete in 48 hours."]].map(([a, b], n) => <p key={a}><i>{n + 1}</i><span><b>{a}</b><small>{b}</small></span></p>)}<blockquote><small>FOUNDER RULE</small>Do not scale activity. Scale proof.</blockquote></section>}
      ...
      {tab === "Quiz" && <section className="quiz">{quiz.map((q, qi) => <article key={q.q}><small>QUESTION {qi + 1}</small><h3>{q.q}</h3><div>{q.a.map((a, ai) => <button className={answers[qi] === ai ? "selected" : ""} key={a} onClick={() => setAnswers({ ...answers, [qi]: ai })}><i>{String.fromCharCode(65 + ai)}</i>{a}</button>)}</div></article>)}{Object.keys(answers).length === quiz.length && <footer><b>{score}/{quiz.length}</b><span>{score === quiz.length ? "Excellent work." : "Review the lesson and try again."}</span></footer>}</section>}
      <footer className="lesson-nav"><button disabled={index === 0} onClick={() => navLesson(all[index - 1])}>← Previous</button><button className={completed.includes(id) ? "done" : "dark"} onClick={() => toggle(id)}>{completed.includes(id) ? "✓ Completed" : "Mark complete"}</button><button disabled={index === all.length - 1} onClick={() => navLesson(all[index + 1])}>Next →</button></footer>
...
function ProductLab({ add }: { add: (product: typeof researchProducts[number]) => void }) {
  const [q, setQ] = useState("");
  const [score, setScore] = useState({ demand: 7, margin: 7, creative: 7, competition: 5 });
  const results = researchProducts.filter((p) => p.name.toLowerCase().includes(q.toLowerCase()));
  ...
  return <><Title eyebrow="LIVE PRODUCT INTELLIGENCE" title="Product Lab" text="Find live demand signals, validate the economics, and only then add a product to your operating plan." action={<a className="dark-btn" target="_blank" rel="noreferrer" href={`https://trends.google.com/trends/explore?q=${encoded}`}>Open Google Trends ↗</a>} /><div className="product-search">⌕<input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search a product or niche, then open the live sources below..." /></div><div className="sources"><a target="_blank" rel="noreferrer" href="https://www.amazon.com/gp/movers-and-shakers">amz <b>Amazon Movers</b> ↗</a><a target="_blank" rel="noreferrer" href="https://ads.tiktok.com/business/creativecenter/inspiration/popular/pc/en">tt <b>TikTok Creative Center</b> ↗</a><a target="_blank" rel="noreferrer" href={`https://www.facebook.com/ads/library/?q=${encoded}`}>meta <b>Meta Ad Library</b> ↗</a><a target="_blank" rel="noreferrer" href={`https://www.aliexpress.us/w/wholesale-${encoded}.html`}>ali <b>Supplier search</b> ↗</a></div><section className="signal-score"><div><small>LIVE RESEARCH WORKFLOW</small><h3>Score your evidence—not your intuition.</h3><p>Use TikTok creative, Amazon velocity, Google search interest and supplier quotes. The score remains your decision record.</p></div><article><small>OPPORTUNITY SCORE</small><b>{totalScore}<span>/100</span></b><p>{totalScore >= 70 ? "Worth a controlled test" : "Gather stronger proof first"}</p></article><div className="score-fields">{([['Demand', 'demand'], ['Margin room', 'margin'], ['Creative potential', 'creative'], ['Competition', 'competition']] as const).map(([label, key]) => <label key={key}><span>{label}<b>{score[key]}/10</b></span><input type="range" min="0" max="10" value={score[key]} onChange={(event) => setScore({ ...score, [key]: Number(event.target.value) })} /></label>)}</div></section><p className="data-note"><i>i</i><span><b>Live-source research desk</b> · This workspace opens the current marketplaces and ad libraries. Curated examples teach the scoring workflow; verify demand, suppliers and saturation before you spend.</span></p><div className="products"><header><span>PRODUCT</span><span>SCORE</span><span>COST / PRICE</span><span>VELOCITY</span><span>SATURATION</span><span /></header>{results.map((p) => <article key={p.name}><p><i>{p.category[0]}</i><span><b>{p.name}</b><small>{p.category}</small></span></p><strong>{p.score}</strong><span>{cash(p.cost)} / {cash(p.price)}</span><b className="green">{p.velocity}</b><em className={p.saturation.toLowerCase()}>{p.saturation}</em><button onClick={() => add(p)}>Add to tracker +</button></article>)}</div></>;
}
...
function Resources() {
  const files = [["PDF", "100-point Shopify launch checklist", "A page-by-page quality gate before publishing."], ["SHEET", "Product validation scorecard", "Score demand, margin, saturation and supplier risk."], ["SCRIPT", "Supplier outreach pack", "Sample request and negotiation follow-up."], ["DOC", "UGC creator brief", "Script, shot list and usage-rights checklist."], ["SHEET", "30-day organic content plan", "Hooks, formats and daily prompts."], ["SOP", "Customer support playbook", "Human response templates for common issues."]];
  const download = (name: string) => { const blob = new Blob([`${name}\n\nEcom Hub Training Resource\n\n1. Current evidence\n2. Decision\n3. Owner\n4. Deadline\n5. Success metric\n`], { type: "text/plain" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `${name.toLowerCase().replaceAll(" ", "-")}.txt`; a.click(); URL.revokeObjectURL(a.href); };
  return <><Title eyebrow="TEMPLATES & SYSTEMS" title="Resource vault" text="Download practical tools that turn each lesson into a repeatable operating system." /><div className="resources">{files.map(([type, name, text], n) => <article key={name}><header><i>{type}</i><span>0{n + 1}</span></header><h3>{name}</h3><p>{text}</p><button onClick={() => download(name)}>Download resource ↓</button></article>)}</div><section className="resource-banner"><div><small>90-DAY PLAN</small><h2>Ready to turn learning into a launch?</h2><p>Follow the weekly build sequence and finish with a measured, customer-ready store.</p></div><button onClick={() => download("Ecom Hub 90-Day Launch Plan")}>Download the plan ↓</button></section></>;
}
